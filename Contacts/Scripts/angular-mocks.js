 ...
 *
 * For extra convenience, `whenRoute` and `expecntoute` shortcuts are available. These methods offer colon
...
function createHttpBackendMock($rootScope, $timeout, $delegate, $browser) {
   ...
   * order to change how a matched request is handled. See #expect for more info.
   */
  $httpBackend.expecntoute = function(method, url) {
    var pathObj = parseRoute(url);
 ...
 *       var cntl = $controller('MyDirectiveController', { /* no locals &#42;/ }, { name: 'Clark Kent' });
 *       cntl.log();
 *
 *       expect(cntl.name).toEqual('Clark Kent');
 ...
 *
 *   myApp.controller('Maincntl', function Maincntl($http) {
 *     var cntl = this;
 *
 *     cntl.phones = [];
 *     cntl.newPhone = {
 ...
 *     };
 *
 *     cntl.getPhones = function() {
 *       $http.get('/phones').then(function(response) {
 *         cntl.phones = response.data;
 *       });
 *     };
 *
 *     cntl.addPhone = function(phone) {
 *       $http.post('/phones', phone).then(function() {
 *         cntl.newPhone = {name: ''};
 *         return cntl.getPhones();
 *       });
 *     };
 *
 *     cntl.getPhones();
 ...
 *   <div ng-controller="Maincntl as $cntl">
 *   <form name="newPhoneForm" ng-submit="$cntl.addPhone($cntl.newPhone)">
 *     <input type="text" ng-model="$cntl.newPhone.name">
 *     <input type="submit" value="Add Phone">
 *   </form>
 *   <h1>Phones</h1>
 *   <ul>
 *     <li ng-repeat="phone in $cntl.phones">{{phone.name}}</li>
 *   </ul>
 *   </div>
 * </file>
 * </example>
...
(function() {
  ...
  window.browserTrigger = function browserTrigger(element, eventType, eventData) {
    ...
    if (/transitionend/.test(eventType)) {
      ...
      x = x || 0;
      y = y || 0;
      evnt.initMouseEvent(eventType, true, true, window, 0, x, y, x, y, pressed('cntl'),
